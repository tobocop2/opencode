...
| <nobr><code>{"--resize-replay"}</code></nobr> |       | Replay `--mini` session history after a terminal resize. Off by default; useful on terminals that do not reflow scrollback on resize, such as plain xterm |
